...
class C2F_ViT_stage(nn.Module):
    ...
    def forward(self, x, y):
        ...
        x = self.image_pyramid(x, self.num_stages)  # 用于生成输入图像的金字塔形式，以便在模型中进行多尺度处理
        ...
        for i in range(self.num_stages):
            ...
            if i > 0:
                xy_patch_embed = xy_patch_embed + xy_fea # 当前阶段计算得到的图像块嵌入与先前阶段的特征进行累加
            ...
            # trans_xyz = affine[0, 0:3]
            # rotate_xyz = affine[0, 3:6] * math.pi
            # shearing_xyz = affine[0, 6:9] * math.pi
            # scaling_xyz = 1 + (affine[0, 9:12] * 0.5)
...
class AffineCOMTransform(nn.Module):
    ...
    def forward(self, x, affine_para, mode = None):
        ...
        if mode is None:
            ...
            transformed_x = F.grid_sample(x, grid, mode='nearest', align_corners=True)
...
class Center_of_mass_initial_pairwise(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # id.shape=torch.Size([1, 3, 4]); x.shape=torch.Size([4(batch_s), 1, 192, 192, 192])
        id_grid = F.affine_grid(self.id, x.shape, align_corners=True)
        # mask = (x > 0).float()
        ...
        grid = F.affine_grid(self.to_center_matrix,
                             x.shape,
                             align_corners=True)
