...
class NICE_Trans(nn.Module):
    ...
    def forward(self, fixed, moving):
        ...
        flow_3_up1, aff_flow_5_up1, affine_para = self.Decoder(x_fix, x_mov)
        warped = self.SpatialTransformer(moving, flow_3_up1)
        affined = self.AffineTransformer(moving, affine_para)
        
        # return warped, flow, affined, affine_para
        return warped, flow_3_up1, affined, affine_para, aff_flow_5_up1
...
class Trans_decoder(nn.Module):
    ...
    def __init__(self,
        ...
        # self.ResizeTransformer16 = ResizeTransformer_block(resize_factor=16, mode='trilinear')
        self.SpatialTransformer = SpatialTransformer_block(mode='bilinear')

    def forward(self, x_fix, x_mov):
        
        x_fix_1, x_fix_2, x_fix_3 = x_fix
        x_mov_1, x_mov_2, x_mov_3 = x_mov
        
        # # Step 1
        # x = torch.cat([x_fix_5, x_mov_5], dim=1)
        # x = self.backdim_5(x)
        # x_5 = self.trans_5(x)
        # aff_flow_5, affine_para = self.reghead_5(x_5)
        
        # # Step 2
        # flow_5_up = self.ResizeTransformer(aff_flow_5) # torch.Size([1, 3, 28, 28, 24])
        ...
        x = self.backdim_3(x)
        x_3 = self.trans_3(x)
        aff_flow, affine_para = self.reghead_3(x_3)
        flow_3 = aff_flow
        ...
        aff_flow_up1 = self.ResizeTransformer(aff_flow)
        aff_flow_up1 = self.ResizeTransformer(aff_flow_up1)
        ...
        return  flow_1, aff_flow_up1, affine_para
...
class SpatialTransformer_block(nn.Module):
    ...
    def forward(self, src, flow):
        ...
        new_locs = new_locs.permute(0, 2, 3, 4, 1) #torch.Size([1, 28, 28, 24, 3])
