...
class NICE_Trans(nn.Module):
    ...
    def forward(self, fixed, moving):
        ...
        current_affine_matrix, affine_para, affine_para_list = self.Decoder(x_fix, x_mov)
        ...
        return affined, current_affine_matrix, affine_para, affine_para_list
...
class Trans_decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.id = torch.zeros((1, 3, 4))
        self.id[0, 0, 0] = 1
        self.id[0, 1, 1] = 1
        self.id[0, 2, 2] = 1

    def matrix2para(self,affine_matrix):
        affine_para=(affine_matrix[:,:3]-self.id).reshape(1,12)
        return affine_para
    ...
    def forward(self, x_fix, x_mov):
        ...
        current_affine_para = self.matrix2para(current_affine_para)
        ...
        # 最终的总仿射变换参数
        affine_para = self.matrix2para(current_affine_para)
        ...
        # 返回最终仿射变换的矩阵、参数；每次分别变换的参数列表
        return  current_affine_matrix, affine_para, affine_para_list
